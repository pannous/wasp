...
typedef chars chars;
typedef const unsigned char *wasm_string;
// wasm strings start with their LEB encoded length and do NOT end with 0 !! :(
...
// ☃ is a single code point but 3 UTF-8 code units (char's), and 1 UTF-16 code unit (char16_t)
...
typedef String grapheme;
// sequence of one or more code points that are displayed as a single 'character' ☀️=☀+'icon' e2 98 80 + ef b8 8f
...
public:
...
#if WASM
			} else if (data + length + 1 == (char*) getHeapEnd()) {// just append recent
				addHeapEnd( byteCount + 1);
#endif
        } else {
    ...
    String operator+(int64 i) {
        return this->operator+(formatLong(i));
    }
    ...
    [[nodiscard("replace generates a new string to be consumed!")]]
    __attribute__((__warn_unused_result__))
    String &replace(chars string, String with) {
        // first only!
    ...
    int in(chars array[]) {
        // array NEEDS to be 0 terminated!!!!
        int i = 0;
        while (array[i]) {
            if (eq(array[i], data))
                return i + 1;

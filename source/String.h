...
//	}
	...
	char charAt(int position) {
		if (position >= length)
			error((String("IndexOutOfBounds at ") + formatLong(position) + String(" in ") + data).data);
...
//    }
	...
	String operator%(double f) {
		String formated = String() + formatLong(f) + String(".") + formatLong((f - int(f)) * 10000);
...
//    [[non-modifying]]
	...
	String &replace(String string, String with) {// first only!
		return replace(string.data, with.data);
	}

...
//	inline String WABT_PRINTF_FORMAT(1, 2) StringPrintf(const char *format, ...) {
//		va_list args;
//		va_list args_copy;
//		va_start(args, format);
//		va_copy(args_copy, args);
...
//	}
	...
	struct Location {
		...
		Location(String filename, int line, int first_column, int last_column)
				: filename(filename),
				  line(line),

...
namespace wabt {
	...
	protected:
		explicit TypeEntry(TypeEntryKind kind,
				...
				: loc(loc), name(name.data), kind_(kind) {}
...
// TODO(binji): Rename this, it is used for more than loads/stores now.
	...
	struct Tag {
		explicit Tag(string_view name) : name(name.data) {}
	...
	struct Global {
		explicit Global(string_view name) : name(name.data) {}
	...
	struct Table {
		explicit Table(string_view name)
				: name(name.data), elem_type(Type::FuncRef) {}
	...
	struct Memory {
		explicit Memory(string_view name) : name(name.data) {}
	...
	struct DataSegment {
		explicit DataSegment(string_view name) : name(name.data) {}

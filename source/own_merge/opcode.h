...
#undef WABT_OPCODE
		...
		Address64 GetMemorySize() const { return GetInfo().memory_size; }
		...
		bool IsNaturallyAligned(Address64 alignment) const;

		// If |alignment| is WABT_USE_NATURAL_ALIGNMENT, return the alignment of
		// |opcode|, else return |alignment|.
		Address64 GetAlignment(Address64 alignment) const;

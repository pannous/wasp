...
namespace wabt {
    namespace {
        ...
        private:
            ...
            Result ReadAlignment(Address64 *align_log2, const char *desc) WABT_WARN_UNUSED;
            ...
            Result ReadAddress(Address64 *out_value, Index memory, const char *desc) WABT_WARN_UNUSED;
...
#pragma clang diagnostic pop
        ...
        Result BinaryReader::ReadU32Leb128(uint32_t *out_value, const char *desc) {
            ...
            if (bytes_read <= 0)
                ERROR_UNLESS(bytes_read > 0, "unable to read u32 leb128 at positon 0x%x: %s", (int) state_.offset,
                         desc);
        ...
        Result BinaryReader::ReadAlignment(Address64 *alignment_log2, const char *desc) {
            uint32_t value;
            CHECK_RESULT(ReadU32Leb128(&value, desc));
            if (value >= 32) {
        ...
        Result BinaryReader::ReadAddress(Address64 *out_value,
                                         Index memory,
                                         const char *desc) {
        ...
        Result BinaryReader::ReadNameSection(Offset section_size) {
            ...
            while (state_.offset < read_end_) {
                ...
                if (i != 0 and name_type != 0) {
                    if (name_type < previous_subsection_type) {
                        PrintError("ERRR %d < %d", name_type, previous_subsection_type);
                    }
                ...
                ERROR_UNLESS(subsection_end <= read_end_, "invalid sub-section size: extends past end");
                ...
                switch (type) {
                    ...
                    case NameSectionSubsection::Function:
                        ...
                        if (subsection_size) {
                            CHECK_RESULT(ReadCount(&num_names, "name section: name count"));
                            ...
                            for (Index j = 0; j < num_names; ++j) {
                                ...
                                ERROR_UNLESS(
                                    last_function_index == kInvalidIndex || function_index > last_function_index,
                    ...
                    case NameSectionSubsection::Local:
                        ...
                        if (subsection_size) {
                            CHECK_RESULT(ReadCount(&num_names, "function count"));
                            CALLBACK(OnLocalNameFunctionCount, num_names);
                            Index last_function_index = kInvalidIndex;
                            for (Index j = 0; j < num_names; ++j) {
                                Index function_index;
                ...
                ERROR_UNLESS(state_.offset == subsection_end,
                             "unfinished name sub-section (type %d:“%s”, names:%d, expected end: 0x%" PRIzx ")", type,
                             GetNameSectionSubsectionName(type), num_names,
                             subsection_end);
            }
        ...
        Result BinaryReader::ReadCustomSection(Index section_index, Offset section_size) {
            ...
            } else if (section_name.indexOf(WABT_BINARY_SECTION_RELOC) == 0) {
        ...
        Result BinaryReader::ReadModule() {
            ...
            ERROR_UNLESS(version == WABT_BINARY_VERSION, "bad wasm file version: %#x (expected %#x)", version,
                         WABT_BINARY_VERSION);

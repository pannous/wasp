...
Code emit_dwarf_debug_info() {
    ...
    code += (byte) DW_OP_WASM_location;
    // := 0xED ;; available DWARF extension code 0x0 0x3, DW_OP_stack_value 0x9f OK ) DW_FORM_exprloc
    ...
    code += (byte) 0x00;
    //DW_AT_decl_file "main.wasp"	("/Users/me/dev/apps/wasp/main.c") // DW_FORM_data1 means just 1 byte
    ...
    code += (byte) 0x1;
    // ? //	DW_AT_frame_base [DW_FORM_exprloc]	// 0… 0x7F ok NO EFFECT 0x80 : Hit the end of input before it was expected
    ...
    code += (byte) 0x00;
    // DW_AT_decl_file[DW_FORM_data1] "main.wasp"	("/Users/me/dev/apps/wasp/main.c") // DW_FORM_data1 means just 1 byte
    ...
    code += (byte) DW_OP_WASM_local; // DW_AT_frame_base / DW_OP_WASM_global_local ?  0,1,2 work 3 hit end 4… fail
    code += (byte) 0x0;
    // ? //	DW_AT_frame_base [DW_FORM_exprloc]	// 0… 0x7F ok, 0x80 : Hit the end of input before it was expected
    //	code += (byte) 0x03; // length of DW_FORM_exprloc:
    //	code += (byte) DW_OP_WASM_location; // := 0xED ;; available DWARF extension code 0x0 0x3, DW_OP_stack_value 0x9f OK ) DW_FORM_exprloc
    ...
    code += (byte) 0x00;
    // DW_AT_decl_file (1) "main.wasp"	("/Users/me/dev/apps/wasp/main.c") // DW_FORM_data1 means just 1 byte
...
Code emit_dwarf_debug_line() {
    ...
    List<byte> md5_checksum = {
        0x66, 0xe6, 0x2f, 0xd1, 0x4c, 0x40, 0x05, 0x88, 0xa9, 0xdd, 0x90, 0xda, 0x38, 0xf1, 0x59,
        0x94
    };
...
Code emit_dwarf_debug_str() {
    ...
    List<String> stringList = {
        "x", "main", "int", "tttt", "j", "/Users/me/wasp/main.c", "int64", "wasp_main", "a",
        "char",
...
Code emit_dwarf_debug_debug_addr() {
    // since DWARF 5
    Code code;
    //DW_OP_addrx
...
String generateMappings(const List<SourceMapping> &mappings_data) {
    ...
    for (auto &mapping: mappings_data) {
        ...
        if (mapping.name_index) {
            /* index zero = NONE, skip optional name */

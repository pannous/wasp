...
pub extern "C" fn wasmtime_anyref_from_i31(
    ...
    // Force the linker to include struct functions using black_box
    std::hint::black_box(wasmtime_anyref_is_struct as usize);
    std::hint::black_box(wasmtime_anyref_as_struct as usize);
    std::hint::black_box(wasmtime_anyref_struct_get_field as usize);
...
/// Check if an `anyref` is a `structref`.
///
/// Returns `true` if the `anyref` is a `structref`, `false` otherwise.
#[unsafe(no_mangle)]
pub extern "C" fn wasmtime_anyref_is_struct(
    cx: WasmtimeStoreContextMut<'_>,
    anyref: Option<&wasmtime_anyref_t>,
) -> bool {
    unsafe {
        anyref
            ...
            .map(|a| a.is_struct(&cx).unwrap_or(false))
            .unwrap_or(false)
    }
}

/// Convert an `anyref` to a `structref` if it is one.
///
/// If the `anyref` is a `structref`, returns `true` and fills `out` with the struct reference.
/// Otherwise returns `false` and `out` is left unchanged.
#[unsafe(no_mangle)]
pub extern "C" fn wasmtime_anyref_as_struct(
    mut cx: WasmtimeStoreContextMut<'_>,
    anyref: Option<&wasmtime_anyref_t>,
    out: &mut MaybeUninit<wasmtime_anyref_t>,
) -> bool {
    unsafe { match anyref.and_then(|a| a.as_wasmtime()) {
        Some(anyref) => {
            match anyref.as_struct(&cx) {
                Ok(Some(structref)) => {
                    let structref_as_anyref = structref.to_anyref();
                    let owned = structref_as_anyref.to_owned_rooted(&mut cx).expect("in scope");
...
///
/// The `anyref` must be a `structref`. Returns `true` and fills `out` with the field value
/// if successful, `false` otherwise (e.g., if the index is out of bounds or the anyref
/// is not a structref).
///
/// Note that i8 and i16 field values are zero-extended into i32 values.
#[unsafe(no_mangle)]
pub extern "C" fn wasmtime_anyref_struct_get_field(
    mut cx: WasmtimeStoreContextMut<'_>,
    anyref: Option<&wasmtime_anyref_t>,
    index: usize,
    ...
    unsafe { match anyref.and_then(|a| a.as_wasmtime()) {
        Some(anyref) => {
            // Try to convert to structref
            match anyref.as_struct(&cx) {
                Ok(Some(structref)) => {
                    // Get the field
                    match structref.field(&mut cx, index) {
                        Ok(val) => {
                            let mut scope = RootScope::new(&mut cx);
                            crate::initialize(out, crate::wasmtime_val_t::from_val(&mut scope, val));
                            true
                        }
                        Err(_) => false,
                    }

...
def value_tree_summary(valueObject, dictionary):
    ...
    numProperties = valueObject.GetChildMemberWithName('properties').GetNonSyntheticValue().GetChildMemberWithName('values').GetChildMemberWithName(
        'values').GetChildMemberWithName('numUsed').GetValue() or "0"
    numChildren = str(valueObject.GetChildMemberWithName('children').GetNonSyntheticValue().GetChildMemberWithName('values').GetChildMemberWithName(
        'numUsed').GetValueAsUnsigned()) or "0"
    return "type: " + valueTreeType + ", " + numProperties + (" property" if numProperties == "1" else " properties") + ", " + numChildren + (
        " child" if numChildren == "1" else " children")

...
auto functionType = 0x60;

auto emptyArray = 0x0;

// https://webassembly.github.io/spec/core/binary/modules.html#binary-module
auto magicModuleHeader = [0x00, 0x61, 0x73, 0x6d];
auto moduleVersion = [0x01, 0x00, 0x00, 0x00];

// https://webassembly.github.io/spec/core/binary/conventions.html#binary-vec
// Vectors are encoded with their length followed by their element sequence
Code encodeVector (any data[]) {
 [
...
Code encodeLocal (number count, Valtype type) {
  return [unsignedLEB128(count), type ];
}

// https://webassembly.github.io/spec/core/binary/modules.html#sections
// sections are encoded by their type followed by their vector contents
Code createSection (Section sectionType, any data[]) {
 [
  sectionType,
  encodeVector(data)
];

Code codeFromProc (ProcStatementNode, TransformedProgram program node)
{

  auto number code[] = [];

  auto symbols = new Map<string, number>(
    node.args.map((arg, index) -> [arg.value, index])
  );

  Code localIndexForSymbol (string name) {
  ...
  Code emitExpression (ExpressionNode node)
{

    traverse(node, (ExpressionNode node) -> {
      switch (node.type) {
        case "numberLiteral":
          code.push(Opcodes.f32_auto);
  ...
  Code emitStatements (StatementNode[] statements) {
      ...
      switch (statement.type) {
        ...
        case "callStatement":
          if (statement.name === "setpixel") {
            ...
            Code index program.findIndex(f {
 f.name === statement.name);
  ...
  auto locals = localCount > 0 ? [encodeLocal(localCount, Valtype.f32)] : [];

  return encodeVector([encodeVector(locals), code, Opcodes.end]);
};;

export Code Emitter emitter (TransformedProgram ast) {
 ...
 [
  ...
  auto typeSection = createSection(
    Section.type,
  ...
  auto codeSection = createSection(
    Section.code,

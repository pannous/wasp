...
parser.AddOption("flag", "help", [&]() {
flag = true;
});
const char *args[] = {"prog name", "--flag"};
parser.Parse(2, const_cast
<char **>(args)
);
EXPECT_EQ(true, flag);
}
...
int count = 0;
OptionParser parser("prog", "desc");
parser.AddOption('f', "flag", "help", [&]() {
++
count;
});
const char *args[] = {"prog name", "-f", "--flag", "-f", "--flag"};
parser.Parse(5, const_cast
<char **>(args)
);
EXPECT_EQ(4, count);
}
...
int count = 0;
OptionParser parser("prog", "desc");
parser.AddOption('a', "a", "help", [&]() {
count += 1; });
parser.AddOption('b', "b", "help", [&]() {
count += 2; });
const char *args[] = {"prog name", "-aa", "-abb"};
parser.Parse(3, const_cast
<char **>(args)
);
EXPECT_EQ(7, count);
}
...
EXPECT_EQ(error,
"prog: unknown option '-f'" ERROR_ENDING);
}
...
EXPECT_EQ(error,
"prog: unknown option '--foo'" ERROR_ENDING);
}
...
EXPECT_EQ("", param);
EXPECT_EQ(error,
"prog: option '--param' requires argument" ERROR_ENDING);
}
...
parser.Parse(1, const_cast
<char **>(args)
);
EXPECT_EQ(error,
"prog: expected arg argument." ERROR_ENDING);
}
...
EXPECT_TRUE(has_x);
EXPECT_EQ(error,
"prog: unexpected argument 'stuff'" ERROR_ENDING);
}
...
EXPECT_EQ(error,
"prog: unexpected argument 'goodbye'" ERROR_ENDING);
}

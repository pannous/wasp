...
namespace wabt {
	...
	namespace {
		...
		Result BinaryReaderIR::OnFunction(Index index, Index sig_index) {
			...
			func.index = index; // named in custom name section
			module_->AppendField(std::move(field));
			return Result::Ok;
		}
...
// KF 2020-12-5 DONT MakeDollarName when READING, only prepend dollar when WRITING!
...
//  return std::string("$") + name.to_string();
		...
		Result BinaryReaderIR::SetFunctionName(Index index, string_view name) {
			...
			std::string dollar_name = GetUniqueName(&module_->func_bindings, MakeDollarName(name));
	...
	Result ReadBinaryIr(const char *filename,
		...
		out_module->loc.filename = filename;

...
EXPECT_EQ(3u, module_desc_.func_types.

size()

);
EXPECT_EQ(1u, module_desc_.imports.

size()

);
EXPECT_EQ(2u, module_desc_.funcs.

size()

);
EXPECT_EQ(1u, module_desc_.tables.

size()

);
EXPECT_EQ(1u, module_desc_.memories.

size()

);
EXPECT_EQ(1u, module_desc_.globals.

size()

);
EXPECT_EQ(0u, module_desc_.tags.

size()

);
EXPECT_EQ(1u, module_desc_.exports.

size()

);
EXPECT_EQ(1u, module_desc_.starts.

size()

);
EXPECT_EQ(1u, module_desc_.elems.

size()

);
EXPECT_EQ(1u, module_desc_.datas.
...
auto host_func =
		HostFunc::New(store_, FuncType{{ValueType::I32},
		                               {ValueType::I32}},
...
mod_ = Module::New(store_, module_desc_);
RefPtr<Trap> trap;
Instance::Instantiate(store_, mod_
.
...
auto after_new = store_.object_count();
EXPECT_EQ(before_new
+ 3, after_new);  // module, instance, func.
...
auto after_new = store_.object_count();
EXPECT_EQ(before_new
+ 6, after_new);  // module, instance, f, t, m, g
